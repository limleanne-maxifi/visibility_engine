...
export function buildUserMessage(data: FormData): string {
  ...
  const lines = [
    ...
    `Competitive standing: ${data.competitiveStanding || 'not specified'}`,
    `Query coverage: ${data.queryCoverage || 'not specified'}`,
    `Platform consistency: ${data.platformConsistency || 'not specified'}`,
